...
- 授权模块：`TLocationPlugin/License/LicenseManager.h/.m`（已加入 Xcode Compile Sources）。
- 定位 Hook 仅在 `[LicenseManager isActivated]` 为真时生效；网络失败按离线容错放行，服务器明确禁用/过期才清除激活状态。
- 部署前必须修改两处占位配置：
  - `kDefaultServerBaseURL` → 你的 HTTPS 域名（如 `https://vca.example.com`，不带末尾斜杠）；
  - `kSignKeyHex` → 与服务器 `VCA_KEY` 完全一致（VPS 上 `openssl rand -hex 32` 生成）。
- 协议：HMAC-SHA256（请求签名 + 响应 response_sign），服务端统一 UTC 时间。
- 服务端与部署手册见同目录 `../license-server/`（尤其 `deploy/DEPLOY.md`）。
